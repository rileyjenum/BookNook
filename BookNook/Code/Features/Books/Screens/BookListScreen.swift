...
struct BookListScreen: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack(spacing: 0) {
                BookshelfView(category: .currentlyReading, height: bookshelfHeight(for: .currentlyReading, in: geometry), selectedBook: $selectedBook)
                BookshelfView(category: .haveRead, height: bookshelfHeight(for: .haveRead, in: geometry), selectedBook: $selectedBook)
                BookshelfView(category: .willRead, height: bookshelfHeight(for: .willRead, in: geometry), selectedBook: $selectedBook)
            }
        }
    }
