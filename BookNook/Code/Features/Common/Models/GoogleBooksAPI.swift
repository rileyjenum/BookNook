...
//        let urlString = "\(baseURL)?q=\(query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")&langRestrict=\(language)&orderBy=\(sort)&key=\(apiKey)&maxResults=40&fields=items(id,volumeInfo(title,authors,description,publisher,publishedDate,pageCount,categories,imageLinks,averageRating,ratingsCount))"
//        print(urlString)
//        
